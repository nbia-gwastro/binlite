...
def normalized_flux_series_from_bad(frequency:float, accretion_series:AccretionSeries, bad:BinaryAlphaDisk, lens_boost=False):
...
def periodic_flux_series_from_bad(frequency:float, accretion_series:AccretionSeries, bad:BinaryAlphaDisk, lens_boost=False):
    ...
    (ndarry) periodic flux timeseries with same shape as accretion_series.primary/secondary/total in cgs
    """
    return bad.fnu_total(frequency) * normalized_flux_series_from_bad(frequency, accretion_series, bad, lens_boost=lens_boost)
